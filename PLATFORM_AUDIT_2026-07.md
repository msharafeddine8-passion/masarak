...
اعتمد التقرير على وكلاء استكشاف قرأوا مقتطفات من الكود، لذلك جرى **التحقّق المستقل** من كل الادّعاءات الحرجة. **أربعة** ادّعاءات من الوكلاء **صحّحناها** بعد الرجوع للكود/الداتا:
...
4. ❌ ادّعاء «7/118 صفحة فقط client-components (111 server)» — **غير صحيح**. grep مباشر: **75/118 صفحة `'use client'`** (43 server). يغيّر قصّة الأداء/الحِمل جوهريًّا (أغلب التطبيق يشحن JS + i18n ويجلب client-side). التفصيل في §9.

---

## 1) الملخّص التنفيذي — Executive Summary

Masarak منصّة تعليميّة **حقيقيّة ومنشورة على الإنتاج** (masaraklb.com)، مبنيّة على Next.js 14 (App Router) + Supabase، عربيّة-أولًا/RTL، بـ**118 صفحة** و13 API route و~55.3k سطر كود. المعماريّة **سليمة وقابلة للتوسّع**: كلّ المحتوى العامّ **ISR/SSG مُخزَّن على CDN** (يخفّف الضغط عن الـDB جذريًّا)، RLS على كل الجداول، دوال `SECURITY DEFINER` مع تحقّقات داخليّة و`search_path` مضبوط، Realtime محصور بالرسائل، rate-limiting موزّع (Upstash) على المسارات الثقيلة، ورؤوس أمان (CSP/HSTS/X-Frame). *(ملاحظة: 75/118 صفحة هي client-components تجلب من Supabase مباشرة — تفصيل الحِمل في §9.)*
...
| **Performance** | 6.5 | محتوى عامّ ISR/CDN + خطوط مُحسّنة + config مُقوّى؛ لكن **75/118 صفحة client** + i18n 476KB (المشكلة #1) + جلب useEffect متسلسل + زمن إيرلندا + 41 `<img>` غير مُحسّنة. |
...
**إيجابيّات مؤكّدة:** خطّ Tajawal عبر `next/font` (لا render-block) · **كلّ المحتوى العامّ ISR/SSG على CDN** · `next.config` مُقوّى (CSP/HSTS/X-Frame/remotePatterns) · TS strict · rate-limiting موزّع (Upstash) لا يفشل مفتوحًا.
**سلبيّات مؤكّدة:** **75/118 صفحة client-components** (تشحن JS + i18n 476KB وتجلب client-side) — أكبر ممّا ادّعى وكيل الفحص (قال 7)؛ waterfalls؛ زمن إيرلندا للجمهور العربيّ.
...
## 9) تقييم التحمّل والسعة تحت الضغط — Load & Capacity Assessment

**البنية التحتيّة الفعليّة:** الواجهة على **Vercel** (Next.js 14) + قاعدة **Supabase** (Postgres 17.6) في منطقة **`eu-west-1` (إيرلندا)** — نقطة مهمّة: جمهور Masarak عربيّ، فكلّ استعلام حيّ يقطع ~50–90ms ذهابًا لإيرلندا. المحتوى الثابت عالميّ عبر CDN، لكن الاستعلامات الديناميكيّة كلّها تعبر للخارج. Sentry مفعّل للمراقبة. الوضع الحالي: أكبر جدول 829 سطرًا، 36 طالبًا — بعيد جدًّا عن أيّ ضغط؛ هذا التقييم **استباقيّ** لأهداف 1k/10k/100k.

### 9.1 خريطة أنماط العرض — أين يقع الحِمل فعليًّا (المفتاح)
| النمط | صفحات | سلوك الحِمل | يتوسّع؟ |
|---|---|---|:---:|
| **ISR / SSG** (`revalidate 1h/24h` + `generateStaticParams`) | كلّ المحتوى العامّ: universities (list+detail), schools, scholarships/[id], majors, careers, guides, glossary, study-abroad, vocational | يُخدَم من **Vercel Edge CDN**، يُعاد توليده كلّ ساعة/يوم — **لا يلمس الـDB لكلّ طلب** | ✅✅ شبه لا محدود |
| **Client (`'use client'`)** — **75/118 صفحة** | لوحات المستخدم/الأدمن/المنظمة/ولي الأمر + الاجتماعي + الأدوات + بعض القوائم العامّة (scholarships list, careers list, universities/[id] body, search) | القشرة CDN، لكن المتصفّح يجلب مباشرةً من **Supabase PostgREST** لكلّ زيارة | ⚠️ مقيّد بحجم compute في Supabase |
| **Dynamic (`force-dynamic`)** | `u/[slug]`, `student/[masarakId]`, صور OG | SSR لكلّ طلب → دالّة Vercel + استعلام | ⚠️ محدود بالتزامن |
| **Edge** | OG images + cron | يتوسّع جيّدًا | ✅ |

**الاستنتاج المُصحَّح:** التقرير كان يقول «111/118 server-components» — **غير دقيق**. الواقع **75 client / 43 server**. لكنّ المُنقِذ الحقيقيّ للحِمل هو أنّ **طبقة المحتوى العامّة كلّها ISR/CDN**، فموجة زوّار SEO أو حملة تسويق تضرب الـCDN لا الـDB.

### 9.2 تصنيف حركة المرور (3 فئات)
1. **زوّار مجهولون / محتوى / SEO** → صفحات ISR على CDN → **يتحمّل ملايين المشاهدات** بلا ضغط DB. ⚠️ استثناء: القوائم العامّة الـclient (scholarships/careers list) تجلب من PostgREST لكلّ زائر.
2. **تطبيق مُصادَق عليه** (dashboard/feed/messages/quiz/tools/org/admin) → 75 صفحة client تجلب من **PostgREST** مباشرة → **عنق الزجاجة = حجم compute في Supabase**، لا Vercel.
3. **نقاط ثقيلة** (دوال Vercel): مسارات AI (career-ai/improve-text/cv-builder) تنادي Anthropic (ثوانٍ، nodejs) · `pdf/generate` (ثقيل) · محميّة بـ**rate-limit** (Upstash Redis: ai 10/دقيقة، form 5/5د، auth 20/دقيقة، general 60/دقيقة، مع fallback في الذاكرة لا يفشل مفتوحًا).

### 9.3 نمذجة السعة (متزامن ≈ 1–5% من المسجّلين وقت الذروة)
| الحجم | ذروة متزامنة تقريبيّة | المحتوى (CDN) | تطبيق (Supabase) | Realtime (messages) | الحكم |
|---|---|---|---|---|---|
| **1k طالب** | ~50 | لا شيء يُذكر | تافه على أيّ tier | ~عشرات WS | ✅ يعمل الآن بلا تغيير |
| **10k طالب + 500 منظمة** | ~300–500 | مريح | جيّد على Supabase **Pro + compute صغير**؛ pagination على feed/community يبدأ يهمّ | ~مئات WS، ضمن حدود Pro | ✅ بشروط بسيطة |
| **100k طالب** | ~2k–5k | مريح | يلزم **compute أكبر (Large/XL) + read replica**؛ pagination إلزاميّ؛ فهارس FK (مُطبَّقة) حاسمة | ~آلاف WS → **يلزم Realtime add-on** | ⚠️ يُبلَغ بترقيات لا إعادة تصميم |

### 9.4 عوائق ونقاط اختناق تحت الضغط
- **حجم compute في Supabase غير مؤكّد** — لم أستطع قراءة الـtier عبر الأدوات؛ **أكّد إضافة compute في لوحة Supabase** (Free/nano يتوقّف عند inactivity ويحدّ الاتصالات؛ 100k يحتاج Large+).
- **Realtime حصريًّا لـ`messages`** (ملفّ واحد) — إيجابيّ: البصمة ضيّقة؛ لكن عند 100k مع رسائل مفتوحة = آلاف WS → **حدّ الاتصالات المتزامنة في الخطّة** يصبح بندًا حقيقيًّا. (الإشعارات/الـfeed ليست Realtime — تُجلَب، فلا تُثقِل WS.)
- **زمن إيرلندا للجمهور العربيّ** — استعلامات client المتسلسلة (waterfalls) تتضاعف: 4 استعلامات × 80ms = 320ms+ شبكة فقط. علاج: **read replica في منطقة أقرب** + دمج الاستعلامات + تحويلها لـserver.
- **قوائم اجتماعيّة بلا pagination** (feed/community/notifications) — تمسح صفوفًا متزايدة؛ تتدهور خطّيًّا مع النموّ. الفهارس (مُطبَّقة) تساعد لكن **الـpagination إلزاميّ عند 10k+**.
- **تكلفة AI تحت الضغط** — إن لم تكن متغيّرات Upstash مضبوطة في الإنتاج، يسقط الحدّ إلى in-memory لكلّ instance → تحت توزّع serverless قد ترتفع فاتورة Anthropic. **أكّد ضبط `UPSTASH_REDIS_REST_URL/TOKEN` في الإنتاج.**
- **`pdf/generate`** (nodejs ثقيل) تحت الضغط → ذاكرة/timeout الدالّة؛ فكّر في طابور/حدّ تزامن.
- **حزمة i18n 476KB** × كلّ صفحة client → عرض نطاق + CPU المتصفّح على نطاق واسع (ليس حِمل خادم، لكنّه يضرب تجربة الموبايل جماعيًّا).

### 9.5 إصلاحات DB (مُطبَّقة في Phase 0 — §12)
**(أ)** 9 فهارس FK على أعمدة كانت table-scan · **(ب)** لفّ 8 سياسات RLS `auth.uid()` → إعادة تقييم مرّة بدل كلّ سطر. كلاهما **مُطبَّق ومُتحقَّق**.

### 9.6 توصيات التحمّل (Load-Hardening) — مرتّبة
1. **[الآن] أكّد:** حجم compute في Supabase + ضبط Upstash في الإنتاج (سطرا حماية حاسمان تحت الضغط).
2. **[10k] pagination** على feed/community/notifications + تحويل جلب الصفحات العامّة الـclient إلى server/ISR (scholarships/careers list) لإزالة استعلام-لكلّ-زائر.
3. **[100k] read replica** (تخفّف Supabase + تحلّ زمن إيرلندا بوضع نسخة أقرب للـMENA) + **Realtime add-on** + رفع compute.
4. **[دائم] راقب** عبر Sentry + Supabase Reports: p95 زمن الاستعلام، تشبّع الـpooler، اتصالات Realtime، أخطاء 429/5xx.

### 9.7 خطّة اختبار الحِمل (Load-Test Plan)
لم يُجرَ اختبار حِمل حيّ (التقييم معماريّ). للتحقّق الفعليّ، شغّل **k6 / Artillery** بسيناريوهات:
- **قراءة محتوى** (مجهول): 5k VU على `/universities`, `/scholarships` → يجب أن يخدمها الـCDN بـp95 < 200ms.
- **جلسة تطبيق** (مُصادَق): 500 VU يفتحون dashboard→feed→messages → راقب p95 استعلام Supabase وتشبّع الـpooler.
- **رشقة AI**: 50 VU على `/api/career-ai` → أكّد أنّ 429 يعمل ولا تتسرّب تكلفة.
- **مقاييس القبول:** p95 < 500ms للتطبيق، 0 أخطاء 5xx، منحنى اتصالات Realtime تحت سقف الخطّة.

**السقف المتوقّع:** المعماريّة **سليمة للتوسّع بلا إعادة تصميم** — ISR للمحتوى + Realtime ضيّق + rate-limiting موزّع + فهارس/RLS (مُطبَّقة). الوصول لـ100k = **ترقيات تشغيليّة** (compute + read replica + Realtime add-on + pagination)، لا إعادة بناء.
...
| **Phase 3 — الأداء والتحمّل والـSEO** | جاهزيّة التوسّع | تقليص i18n (M1)، تحويل جلب useEffect لـserver (P2)، SSR لصفحات التفصيل CSR (M6/S1)، next/image، canonicals + JobPosting. **تحمّل (§9): pagination على القوائم الاجتماعيّة، تأكيد compute/Upstash، read replica + Realtime add-on عند الاقتراب من 100k، تشغيل k6/Artillery.** | قبل التسويق العدواني/النموّ العضويّ. |
...
- **تحقّق مضادّ للوكلاء:** صُحِّحت **4** ادّعاءات خاطئة/مبالغ فيها (البريميوم/الـRPC، روستر المرشد، ربط ولي الأمر، عدّ client/server) بالرجوع للكود والداتا — انظر «ملاحظة منهجيّة» أعلاه.
- **حقائق البنية التحتيّة للتحمّل (§9) مُتحقَّقة من الكود/الإعدادات:** منطقة Supabase `eu-west-1` + PG17 (عبر `get_project`)؛ أنماط العرض ISR/client/dynamic/edge؛ Realtime في `messages` فقط؛ rate-limit عبر Upstash (`lib/ratelimit.ts`)؛ 75/118 client (grep مباشر).
- **قيود:** لم يُشغَّل **اختبار حِمل حيّ** (k6/Artillery) — تقييم التحمّل معماريّ مبنيّ على الحقائق أعلاه؛ خطّة الاختبار في §9.7. ولم يُشغَّل E2E لكلّ صفحة. تدفّقات الدفع خامدة (0 اشتراك). **حجم compute في Supabase لم يتأكّد عبر الأدوات → أكّده في اللوحة.**
- **البناء:** إصلاحات Phase 0 على مستوى DB فقط، تُتحقَّق عبر إعادة تشغيل `get_advisors` بعد التطبيق (لا حاجة لبناء الواجهة).

---
*انتهى التقرير. Phase 0 (فهارس + لفّ RLS + search_path) **مُطبَّقة ومُتحقَّقة على الإنتاج**. الخطوة التالية المقترحة: Phase 1 (المشاكل الحرجة C1–C4)، وقبل التوسّع الكبير تنفيذ توصيات التحمّل §9.6 + اختبار الحِمل §9.7.*
