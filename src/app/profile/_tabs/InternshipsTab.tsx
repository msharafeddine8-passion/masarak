...
const STATUS_KEYS: Record<string, { labelKey: TranslationKey; color: string; icon: string }> = {
  ...
  rejected:  { labelKey: 'pt.int.s.rejected',  color: 'bg-red-100 text-red-700',           icon: '❌' },
};
...
export default function InternshipsTab({ userId }: { userId: string }) {
  ...
  const remove = async (id: number) => {
    if (!confirm(t('pt.int.confirm_delete'))) return;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon="📋" label={t('pt.int.stat.total')}     value={stats.total}     color="bg-blue-50 text-blue-700" />
        <StatCard icon="📨" label={t('pt.int.stat.applied')}   value={stats.applied}   color="bg-purple-50 text-purple-700" />
        <StatCard icon="🎤" label={t('pt.int.stat.interview')} value={stats.interview} color="bg-amber-50 text-amber-700" />
        <StatCard icon="🏆" label={t('pt.int.stat.offer')}     value={stats.offer}     color="bg-emerald-50 text-emerald-700" />
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <Link href="/tools/cv-builder" className="bg-gradient-to-br from-blue-500 to-blue-700 text-white rounded-2xl p-5 hover:scale-105 transition">
          ...
          <div className="font-bold">{t('pt.int.tool.cv')}</div>
          <div className="text-xs opacity-80 mt-1">{t('pt.int.tool.cv_d')}</div>
        </Link>
        <Link href="/tools/interview-prep" className="bg-gradient-to-br from-purple-500 to-pink-600 text-white rounded-2xl p-5 hover:scale-105 transition">
          <div className="text-3xl mb-2">🎤</div>
          <div className="font-bold">{t('pt.int.tool.interview')}</div>
          <div className="text-xs opacity-80 mt-1">{t('pt.int.tool.interview_d')}</div>
        </Link>
        <Link href="/internships/hub" className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-2xl p-5 hover:scale-105 transition">
          <div className="text-3xl mb-2">🔍</div>
          <div className="font-bold">{t('pt.int.tool.hub')}</div>
          <div className="text-xs opacity-80 mt-1">{t('pt.int.tool.hub_d')}</div>
        </Link>
      </div>

      {/* Add */}
      <div className="flex justify-between items-center">
        <h3 className="font-bold text-xl text-[#1b3a6b]">{t('pt.int.title')}</h3>
        <button onClick={() => setEditing({ company: '', role: '', status: 'planning' })} className="px-5 py-2.5 bg-[#1b3a6b] text-white rounded-lg font-bold text-sm">{t('pt.int.add')}</button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-bold">{t('pt.int.col.date')}</th>
                <th className="px-4 py-3 text-right font-bold">{t('pt.int.col.status')}</th>
                ...
                return (
                  <tr key={a.id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center whitespace-nowrap">
                      ...
                      <button onClick={() => remove(a.id)} className="text-red-600 hover:bg-red-50 px-2 py-1 rounded text-xs font-bold mr-1">{t('pt.int.delete')}</button>
                    </td>
                  </tr>
                );
              })}
