      ...
      {loading ? <div className="text-center py-12">⏳</div> : current.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {current.map((it: any) => {
            ...
            return (
              <div key={it.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group hover:shadow-lg transition">
                {data.photo && <div className="aspect-video bg-slate-100">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={data.photo} alt={data.name} className="w-full h-full object-cover" /></div>}
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1">
                      ...
                      {data.region && <div className="text-xs text-slate-500">{data.region}</div>}
                    </div>
