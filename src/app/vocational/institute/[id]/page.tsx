...
import { getInstituteById } from "../../data";
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center" dir="rtl">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-[#1b3a6b] mb-2">المعهد غير موجود</h1>
          <Link href="/vocational" className="px-5 py-2.5 bg-[#1b3a6b] text-white rounded-lg font-bold mt-4 inline-block">
            ← العودة
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-gray-50 pb-20" dir="rtl">
      <section className="bg-gradient-to-br from-[#1b3a6b] to-[#2d5391] text-white">
        <div className="max-w-4xl mx-auto px-4 py-12">
          <Link href="/vocational" className="text-white/85 hover:text-white text-sm">← كل المعاهد</Link>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-5 mt-4">
            ...
            <div className="flex-1">
              ...
              <span className={`${typeColor[inst.type]} px-3 py-1 rounded-full text-sm font-semibold`}>{inst.type}</span>
            </div>
            {inst.website && (
              <a href={inst.website} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-white text-[#1b3a6b] rounded-lg font-bold text-sm">
                🌐 الموقع
              </a>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {inst.specialties.map((s, i) => (
              <div key={i} className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg text-sm">
