...
// Client island for the institute page. `initialInst` is passed from the server
// component so the full profile renders into the initial SSR HTML (SEO — the body
// was previously fetched in useEffect, so crawlers saw only a ⏳ skeleton). We
// still refetch by id on mount to pick up live DB edits.
export default function InstituteDetailClient({ initialInst, id }: { initialInst: any | null; id: number }) {
  const { t } = useI18n();
  const [inst, setInst] = useState<any | null>(initialInst);
  const [loading, setLoading] = useState(!initialInst);
  const [error, setError] = useState<unknown>(null);

  // FIX (audit C1): .then had no .catch → a failed fetch hung on ⏳ forever.
  // With server-provided initialInst we never flash the skeleton nor clobber good
  // content on a failed refresh — the error only surfaces when we had no data.
  const loadInst = useCallback(() => {
    setError(null);
    fetchInstituteById(id)
      .then((i) => { if (i) setInst(i); setLoading(false); })
      .catch((e) => { if (!initialInst) setError(e); setLoading(false); });
  }, [id, initialInst]);
