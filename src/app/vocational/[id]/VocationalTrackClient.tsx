...
// Client island for the vocational track page. `initialTrack` is passed from the
// server component so the full profile renders into the initial SSR HTML (SEO —
// the body was previously fetched in useEffect, so crawlers saw only a ⏳ skeleton).
// We still refetch by id on mount to pick up live DB edits.
export default function VocationalTrackClient({ initialTrack, id }: { initialTrack: any | null; id: string }) {
  const { t, dir } = useI18n();
  const [track, setTrack] = useState<any | null>(initialTrack);
  const [loading, setLoading] = useState(!initialTrack);
  const [error, setError] = useState<unknown>(null);

  // FIX (audit C1): .then had no .catch → a failed fetch hung on ⏳ forever.
  // With server-provided initialTrack we never flash the skeleton nor clobber
  // good content on a failed refresh — the error only surfaces when we had no data.
  const loadTrack = useCallback(() => {
    setError(null);
    fetchTrackById(id)
      .then((x) => { if (x) setTrack(x); setLoading(false); })
      .catch((e) => { if (!initialTrack) setError(e); setLoading(false); });
  }, [id, initialTrack]);
