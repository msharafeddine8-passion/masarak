    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center" dir="rtl">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-[#1b3a6b] mb-2">المسار غير موجود</h1>
          <p className="text-gray-600 mb-6">لم نجد المسار المهني بالمعرّف "{id}".</p>
          <Link href="/vocational" className="px-5 py-2.5 bg-[#1b3a6b] text-white rounded-lg font-bold hover:bg-[#142d54]">
            ← العودة لكل المسارات
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-gray-50 pb-20" dir="rtl">
      ...
      <section className="bg-gradient-to-br from-[#1b3a6b] to-[#2d5391] text-white">
        <div className="max-w-5xl mx-auto px-4 py-12">
          <Link href="/vocational" className="inline-flex items-center gap-2 text-white/85 hover:text-white text-sm mb-4">
            ← كل المسارات المهنية
          </Link>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2 text-sm">
                <span className="bg-white/15 backdrop-blur px-3 py-1 rounded-full font-semibold">{levelLabels[track.level] || track.level}</span>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {track.subjects.map((s, i) => (
              <div key={i} className="flex items-start gap-2 text-sm text-gray-700 bg-gray-50 p-3 rounded-lg">
                <span className="text-[#1b3a6b] font-bold flex-shrink-0">📖</span>
                <span>{s}</span>
              </div>
            ))}
          </div>
        ...
        {track.uniEquiv && (
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6">
            <h2 className="font-bold text-lg text-[#1b3a6b] mb-2 flex items-center gap-2">🎓 معادلة جامعية</h2>
            <p className="text-gray-700 text-sm">{track.uniEquiv}</p>
          </div>
        )}

        {/* CTA */}
        <div className="bg-gradient-to-br from-[#1b3a6b] to-[#2d5391] text-white rounded-2xl p-8 text-center">
          <h2 className="text-2xl font-extrabold mb-2">جاهز تبدأ؟</h2>
          <p className="text-white/85 mb-5">شوف المعاهد اللي بتقدّم هذا المسار</p>
          <Link href="/vocational" className="inline-block px-6 py-3 bg-white text-[#1b3a6b] rounded-lg font-bold">
            استعرض المعاهد ←
          </Link>
        </div>
      </div>
