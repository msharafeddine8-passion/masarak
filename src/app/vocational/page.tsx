  ...
  return (
    <main className="min-h-screen bg-bg pb-20" dir={dir}>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-6">
        {/* View Switcher */}
        <div className="bg-surface rounded-2xl shadow-md p-3 mb-4 flex gap-2">
          <button
        ...
        {view === 'tracks' && (
          <>
            ...
            <div className="text-sm text-ink-muted mb-3">{filteredTracks.length} {t('voc.count.label')}</div>
...
function TrackCard({ track }: { track: VocationalTrack }) {
  ...
  const demandColor = {
    ...
    "منخفض": "bg-bg-soft text-ink-muted",
  ...
  return (
    <Link href={`/vocational/${track.id}`} className="bg-surface rounded-2xl border border-white/10 hover:border-[#1b3a6b] hover:shadow-lg transition p-5 block group">
      ...
      <p className="text-xs text-ink-subtle mb-3">{track.sector}</p>
      <p className="text-sm text-ink-muted line-clamp-2 mb-3">{track.desc}</p>
      <div className="border-t pt-3 space-y-1 text-xs">
        <div className="flex justify-between"><span className="text-ink-subtle">{t('voc.card.duration')}</span> <span className="font-semibold">{track.duration}</span></div>
        <div className="flex justify-between"><span className="text-ink-subtle">{t('voc.card.salary_lb')}</span> <span className="font-semibold text-emerald-600">{track.salaryLB}</span></div>
        <div className="flex justify-between"><span className="text-ink-subtle">{t('voc.card.salary_gulf')}</span> <span className="font-semibold text-blue-600">{track.salaryGulf}</span></div>
      </div>
...
function InstituteCard({ inst }: { inst: Institute }) {
  ...
  return (
    <Link href={`/vocational/institute/${inst.id}`} className="bg-surface rounded-2xl border border-white/10 hover:border-[#1b3a6b] hover:shadow-lg transition p-5 block group">
      ...
      <p className="text-xs text-ink-subtle mb-3">{inst.region}</p>
      <div className="flex flex-wrap gap-1 mb-3">
        {inst.specialties.slice(0, 4).map((s, i) => (
          <span key={i} className="text-xs bg-bg-soft text-ink-muted px-2 py-1 rounded">{s}</span>
        ))}
      </div>
      <span className="text-[#1b3a6b] font-bold text-sm">{t('voc.card.details')}</span>
    </Link>
