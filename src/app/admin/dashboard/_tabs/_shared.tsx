...
export function Input(p: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...p} className="w-full px-3 py-2 border border-white/10 rounded-lg text-sm focus:border-[#1b3a6b] focus:outline-none focus:ring-2 focus:ring-[#1b3a6b]/10" />;
}
export function Textarea(p: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...p} className="w-full px-3 py-2 border border-white/10 rounded-lg text-sm min-h-[80px] focus:border-[#1b3a6b] focus:outline-none" />;
}
export function Select({ children, ...p }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...p} className="w-full px-3 py-2 border border-white/10 rounded-lg bg-surface text-sm focus:border-[#1b3a6b] focus:outline-none">{children}</select>;
}
  ...
  return (
    <div className="bg-surface rounded-2xl border border-slate-100 p-4 mb-5 flex flex-wrap gap-3 items-center">
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 بحث..."
        className="flex-1 min-w-[200px] px-4 py-2.5 border border-white/10 rounded-lg text-sm" />
...
export function EmptyState({ icon = '📂', text, action }: { icon?: string; text: string; action?: React.ReactNode }) {
  return (
    <div className="text-center py-16 bg-surface rounded-2xl border-2 border-dashed border-white/10">
