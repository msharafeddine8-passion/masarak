...
export function Field({ label, children, span }: { label: string; children: React.ReactNode; span?: number }) {
  return <div className={span ? `md:col-span-${span}` : ''}><label className="block text-xs font-semibold text-slate-700 mb-1.5">{label}</label>{children}</div>;
}
export function Input(p: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...p} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:border-[#1b3a6b] focus:outline-none focus:ring-2 focus:ring-[#1b3a6b]/10" />;
}
export function Textarea(p: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...p} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm min-h-[80px] focus:border-[#1b3a6b] focus:outline-none" />;
}
export function Select({ children, ...p }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...p} className="w-full px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm focus:border-[#1b3a6b] focus:outline-none">{children}</select>;
}
...
export function Modal({ onClose, title, children, size = 'lg' }: { onClose: () => void; title: string; children: React.ReactNode; size?: 'sm' | 'md' | 'lg' | 'xl' }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className={`bg-white rounded-2xl shadow-2xl w-full ${sizes[size]} max-h-[92vh] overflow-y-auto`} dir="rtl">
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-xl font-bold text-[#1b3a6b]">{title}</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-full hover:bg-slate-100 flex items-center justify-center text-2xl">×</button>
        </div>
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-lg">
          ...
          <div className="flex-1 text-xs text-slate-500 break-all" dir="ltr">{value}</div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-4 mb-5 flex flex-wrap gap-3 items-center">
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 بحث..."
        className="flex-1 min-w-[200px] px-4 py-2.5 border border-slate-200 rounded-lg text-sm" />
      <div className="text-sm text-slate-500 font-semibold">{count}{total !== undefined && total !== count ? ` / ${total}` : ''}</div>
      {extra}
