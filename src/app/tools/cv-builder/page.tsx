...
export default function CVBuilderPage() {
  ...
  const inp  = "w-full border border-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all bg-surface";
  const lbl  = "block text-xs font-semibold text-text-sub mb-1";
  const card = "bg-surface rounded-xl border border-line shadow-sm p-5";
        ...
        <div className="max-w-screen-xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCV(SAMPLE_CV)}
              className="text-sm border border-line px-3 py-1.5 rounded-lg hover:border-primary text-text-sub hover:text-primary transition-all">
              {t('cv.btn.sample')}
            </button>
            <button onClick={() => setCV(EMPTY_CV)}
              className="text-sm border border-line px-3 py-1.5 rounded-lg hover:border-red-300 text-text-sub hover:text-red-500 transition-all">
              {t('cv.btn.clear')}
            </button>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
          ...
          {TEMPLATES.map(tpl => (
            <button key={tpl.id} onClick={() => setTmpl(tpl.id)}
              className={`px-4 py-2 rounded-xl text-sm font-bold border-2 transition-all flex flex-col items-center gap-0.5 ${
                template === tpl.id ? "border-primary bg-primary text-white" : "border-line text-text-sub hover:border-primary bg-surface"
              }`}>
              <span>{tpl.label}</span>
        ...
        <div className="grid xl:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {/* Tab bar */}
            <div className="flex gap-1 bg-surface rounded-xl p-1 border border-line shadow-sm overflow-x-auto">
              {TABS.map(tb => (
          ...
          <div className="xl:sticky xl:top-24 xl:h-[calc(100vh-120px)] overflow-y-auto">
            ...
            <div className="bg-surface shadow-2xl rounded-xl overflow-hidden border border-line">
