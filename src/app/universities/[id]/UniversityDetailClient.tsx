...
// Client island for the university detail page. Core data (`initialUni`) is
// passed from the server component so the full profile renders into the initial
// SSR HTML (SEO — previously the body was fetched in useEffect, so crawlers only
// saw a ⏳ skeleton). We still refetch by id on mount to pick up live DB edits.
export default function UniversityDetailClient({ initialUni, id }: { initialUni: any | null; id: number }) {
  const { t, dir, lang } = useI18n();
  const [uni, setUni] = useState<any | null>(initialUni);
  ...
  // With server-provided initialUni we never flash the skeleton nor clobber good
  // content on a failed refresh — the error only surfaces when we had no data.
  const loadUni = useCallback(() => {
    setError(null);
    fetchUniversityById(id)
      .then((u) => { if (u) setUni(u); setLoading(false); })
      .catch((e) => { if (!initialUni) setError(e); setLoading(false); });
  }, [id, initialUni]);
