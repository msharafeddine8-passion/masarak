...
export default function UniversityDetailPage() {
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center" dir="rtl">
        <div className="text-center max-w-md">
          ...
          <Link href="/universities" className="px-5 py-2.5 bg-[#1b3a6b] text-white rounded-lg font-bold hover:bg-[#142d54]">
            ← العودة لكل الجامعات
          </Link>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-20" dir="rtl">
      {/* Hero */}
      <section className={`relative bg-gradient-to-br ${uni.color} text-white`}>
        {uni.photo && (
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-12">
          <Link href="/universities" className="inline-flex items-center gap-2 text-white/85 hover:text-white text-sm mb-4">
            ← كل الجامعات
          </Link>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-5">
            ...
            <div className="flex flex-col gap-2">
              <a href={uni.url} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-white text-[#1b3a6b] rounded-lg font-bold text-sm text-center hover:opacity-90">
                🌐 الموقع الرسمي
              </a>
              {uni.scholarships && (
                <Link href="/scholarships" className="px-5 py-2.5 bg-[#5cc4b8] text-[#1b3a6b] rounded-lg font-bold text-sm text-center">
                  🎓 منح متاحة
                </Link>
              )}
            </div>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-5 grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
          <Stat label="الطلاب" value={uni.students.toLocaleString()} />
          <Stat label="الكليات" value={uni.faculties} />
          <Stat label="الرسوم/سنة" value={uni.tuitionMin === 0 ? 'مجاني' : `$${uni.tuitionMin.toLocaleString()}–${uni.tuitionMax.toLocaleString()}`} />
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="flex overflow-x-auto border-b">
            {([
              ...
              { id: 'reviews', label: 'تقييمات الطلاب', icon: '⭐' },
          ...
          <div className="p-6 md:p-8">
            {tab === 'overview' && (
              <div className="space-y-6">
                <div>
                  <h3 className="font-bold text-lg text-[#1b3a6b] mb-2">عن الجامعة</h3>
                  <p className="text-gray-700 leading-relaxed">{uni.desc}</p>
                </div>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <InfoRow label="عدد الطلاب" value={uni.students.toLocaleString()} />
                </div>
                <div>
                  <h3 className="font-bold text-lg text-[#1b3a6b] mb-3">المسارات الرئيسية</h3>
                  <div className="flex flex-wrap gap-2">
                    {uni.paths.map((p) => (
                      <span key={p} className="px-3 py-1.5 bg-blue-50 text-[#1b3a6b] rounded-full text-sm font-semibold">
                        {p}
                      </span>
                    ))}
                  </div>
            ...
            {tab === 'majors' && (
              <div>
                <h3 className="font-bold text-lg text-[#1b3a6b] mb-4">التخصصات المتوفرة ({uni.majors.length})</h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {uni.majors.map((m, i) => (
                    <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-semibold text-gray-700">
                      📚 {m}
                    </div>
                  ))}
                </div>
            ...
            {tab === 'reviews' && (
              <div>
                ...
                {reviews.length === 0 ? (
                  <div className="text-center text-gray-400 py-8">لا توجد تقييمات بعد</div>
                ) : (
              ...
              <div className="space-y-5">
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
                  ...
                  <div className="grid md:grid-cols-2 gap-3 text-sm">
                    ...
                    <InfoRow label="الرسوم" value={uni.tuitionMin === 0 ? 'مجانية تقريباً' : `$${uni.tuitionMin}–${uni.tuitionMax}/سنة`} />
                    <InfoRow label="منح دراسية" value={uni.scholarships ? '✓ متوفرة' : '✗ غير متوفرة'} />
                  </div>
                </div>
                <div className="flex flex-wrap gap-3">
                  <a href={uni.url} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-[#1b3a6b] text-white rounded-lg font-bold text-sm">
                    🌐 ابدأ تقديم طلب
                  </a>
                  <Link href="/tools/cost-calculator" className="px-5 py-2.5 border-2 border-[#1b3a6b] text-[#1b3a6b] rounded-lg font-bold text-sm">
                    💰 احسب الكلفة
                  </Link>
                  <Link href="/scholarships" className="px-5 py-2.5 border-2 border-[#5cc4b8] text-[#1b3a6b] rounded-lg font-bold text-sm">
                    🎓 ابحث عن منح
                  </Link>
                </div>
              </div>
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div>
      <div className="text-2xl font-extrabold text-[#1b3a6b]">{value}</div>
