    ...
    <div className="min-h-screen bg-light" dir="rtl">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <div className={`bg-gradient-to-br ${uni.color} rounded-2xl p-6 md:p-10 mb-6 text-white relative overflow-hidden`}>
          <div className="absolute top-0 right-0 opacity-10 text-[12rem] leading-none -mt-8 -mr-4 select-none">{uni.emoji}</div>
          <div className="relative z-10">
            <div className="flex items-start gap-4 mb-4">
              <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center text-4xl flex-shrink-0">{uni.emoji}</div>
              <div>
        ...
        {tab === "overview" && (
          <div className="space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div className="card">
                <h3 className="font-bold text-primary mb-4 flex items-center gap-2"><span>📊</span> إحصائيات سريعة</h3>
              ...
              <div className="card">
                <h3 className="font-bold text-primary mb-4 flex items-center gap-2"><span>💡</span> مميزات الجامعة</h3>
            ...
            <div className="card">
              ...
              <div className="flex flex-wrap gap-3">
                <a href={uni.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors">🌐 الموقع الرسمي ↗</a>
                <a href={`${uni.url}/admissions`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white border-2 border-primary text-primary px-4 py-2 rounded-xl text-sm font-semibold hover:bg-primary/5 transition-colors">📝 شروط القبول ↗</a>
                {uni.scholarships && (
                  <a href={`${uni.url}/scholarships`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-amber-50 border-2 border-amber-300 text-amber-700 px-4 py-2 rounded-xl text-sm font-semibold hover:bg-amber-100 transition-colors">🏆 المنح الدراسية ↗</a>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="mt-5 p-4 bg-blue-50 rounded-xl border border-blue-100">
              <p className="text-sm text-blue-700">
                ...
                <a href={`${uni.url}/academics`} target="_blank" rel="noopener noreferrer" className="font-bold underline">الموقع الرسمي لـ {uni.short}</a>
              </p>
            </div>
          </div>
        )}
            ...
            <div className="card">
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="bg-primary/10 rounded-xl p-4 text-center">
                  <div className="text-2xl font-extrabold text-primary">{uni.tuitionMin === 0 ? "مجاني" : `$${uni.tuitionMin.toLocaleString()}`}</div>
                  <div className="text-xs text-text-sub mt-1">الحد الأدنى / سنة</div>
                </div>
                <div className="bg-accent/10 rounded-xl p-4 text-center">
                  <div className="text-2xl font-extrabold text-accent">{uni.tuitionMax === 500 ? "~$500" : `$${uni.tuitionMax.toLocaleString()}`}</div>
                  <div className="text-xs text-text-sub mt-1">الحد الأقصى / سنة</div>
                </div>
              </div>
              <div className="space-y-3 text-sm text-text-sub">
                <div className="flex items-start gap-2 bg-gray-50 rounded-xl p-3"><span>📌</span><p>الأرقام تقديرية وتشمل الرسوم الأكاديمية فقط. قد تضاف رسوم التسجيل والمختبرات والنشاطات.</p></div>
                <div className="flex items-start gap-2 bg-gray-50 rounded-xl p-3"><span>💱</span><p>تدفع الرسوم في بعض الجامعات بالليرة اللبنانية وفق سعر صرف رسمي — تحقق مع الجامعة مباشرة.</p></div>
                {uni.tuitionMin === 0 && (
                  <div className="flex items-start gap-2 bg-green-50 rounded-xl p-3 text-green-700"><span>✅</span><p>الجامعة اللبنانية تعليم حكومي شبه مجاني متاح لجميع حاملي البكالوريا.</p></div>
                )}
              </div>
            </div>
            {uni.scholarships && (
              <div className="card border-2 border-amber-200 bg-amber-50/50">
                <h3 className="font-bold text-amber-700 text-lg mb-3 flex items-center gap-2"><span>🏆</span> المنح الدراسية المتاحة</h3>
                <div className="space-y-2 text-sm text-amber-800">
                  {["منح الامتياز الأكاديمي (للمتفوقين في البكالوريا)","منح الحاجة المالية (للأسر ذات الدخل المحدود)","منح النشاطات اللاصفية والقيادة الطلابية","منح أبناء الخريجين","منح التبادل الطلابي مع جامعات دولية"].map((s, i) => (
                    <div key={i} className="flex items-center gap-2 bg-amber-50 rounded-xl px-3 py-2"><span className="text-amber-500">✦</span> {s}</div>
                  ))}
                </div>
                <a href={`${uni.url}/scholarships`} target="_blank" rel="noopener noreferrer" className="mt-4 block bg-amber-500 text-white text-center py-3 rounded-xl font-bold text-sm hover:bg-amber-600 transition-colors">تقدم للمنح الدراسية ↗</a>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-5">
            ...
            <div className="card">
              ...
              <form onSubmit={submitReview} className="space-y-3">
                ...
                <button type="submit" className="btn-primary px-6 py-2 rounded-xl text-sm font-semibold">نشر التقييم ←</button>
              </form>
            </div>
            <div className="space-y-3">
              {reviews.map((r, i) => (
        ...
        <div className="card mt-6 bg-gradient-to-r from-primary to-[#1e4080] text-white text-center py-8 rounded-2xl">
          ...
          <Link href="/universities" className="bg-white text-primary font-bold px-6 py-2 rounded-xl text-sm hover:bg-white/90 transition-colors inline-block">مقارنة الجامعات ←</Link>
        </div>
      </main>
    </div>
  );
