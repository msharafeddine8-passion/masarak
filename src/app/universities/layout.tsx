...
export default function UniversitiesLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      {children}
      <nav aria-label="كل الجامعات" className="bg-white border-t border-gray-200 py-10" dir="rtl">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-2xl font-extrabold text-[#012730] mb-2">كل الجامعات (تصفّح سريع)</h2>
          <p className="text-sm text-gray-600 mb-5">٣٥ جامعة معتمدة — اضغط الاسم للذهاب لصفحة التفاصيل</p>
            ...
            {UNIVERSITIES.map(u => (
              <li key={u.id}>
                ...
                >
                  <span>{u.emoji}</span>
                  <span>{u.name}</span>
                  <span className="text-xs text-gray-400 font-normal">— {u.region}</span>
                </Link>
              </li>
            ))}
          </ul>
