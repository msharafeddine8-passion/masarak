...
export default function OrgMessagesSection({ orgId, currentUserId }: { orgId: string; currentUserId: string }) {
  ...
  return (
    <section id="messaging" className="bg-white rounded-2xl border-2 border-gray-100 p-4 lg:p-6 mb-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-extrabold text-primary">💬 المحادثات مع الطلاب</h3>
        <button onClick={load} className="text-xs font-bold bg-white border-2 border-gray-200 rounded-lg px-3 py-1.5">🔄</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-4 h-[500px]">
          {/* Threads list */}
          <div className="border-2 border-gray-100 rounded-xl overflow-y-auto">
            {threads.map(t => {
          ...
          {/* Active thread */}
          <div className="border-2 border-gray-100 rounded-xl flex flex-col">
            {!activeThread ? (
                  ...
                  {activeMessages.map(m => (
                    <div key={m.id} className={'flex ' + (m.sender_type === 'org' ? 'justify-end' : 'justify-start')}>
                      <div className={'max-w-[80%] rounded-2xl px-3 py-2 text-sm ' + (
                        m.sender_type === 'org' ? 'bg-primary text-white' : 'bg-gray-100 text-ink'
                      )}>
                        <div>{m.body}</div>
                ...
                <div className="border-t border-gray-100 p-3">
                  <div className="flex gap-2">
                    <textarea value={draft} onChange={e => setDraft(e.target.value)}
                      ...
                      className="flex-1 px-3 py-2 rounded-xl border-2 border-gray-100 focus:border-primary outline-none text-sm resize-none" />
